...
$$
\pi_{ARA}(x) = -\frac{u''(x)}{u'(x)} \label{riskaversion}
$$
...
Consider the earlier example, illustrated in @fig-graphical-risk-prem, where the decision involves a choice between a certain payment and a gamble with a 50% probability each for outcomes 0.5 and 1.5:

The risk premium $ \rho $ is the difference between the certain amount of 1 and the amount that equates to this utility level:

$$
\ln(1-\rho) = \ln(\sqrt{0.75}),
$$

resulting in:

$$
\rho = 1 - \sqrt{0.75} \approx 0.14.
$$

This quantification of $ \rho $ as a risk premium measure underscores its relevance and frequent use in financial analyses.
...
where $\rho$ is the CARA coefficient. Given that $W_1$ is a linear transformation of a normally distributed variable, it too follows a normal distribution. Consequently, $u(W_1)$ is log-normally distributed. This leads to the expected utility $U$ as:

$$
U = \mathbb{E}[u(W_1)] = -e^{-\rho \mathbb{E}[W_1] - \frac{\rho}{2} \text{var}(W_1)}  \label{forvNytte}
$$

Key conclusions include:
1. Increased uncertainty regarding future wealth ($\text{var}(W_1)$) diminishes utility.
2. Higher risk aversion ($\rho$) amplifies the adverse impact of uncertainty.
...
$$
a = \frac{(mu - r)}{\rho \sigma^2}  \label{optPort1}
$$
