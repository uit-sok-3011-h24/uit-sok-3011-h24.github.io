...
1. that are most traded (sorted on sum of `Turnover`)
2. has been traded at the last day of the sample
...
res = (
    ...
    .groupby(['ISIN', 'Name'], as_index=False)
    .agg({'Turnover': 'sum'})
    .sort_values(by='Turnover', ascending=False)
    .reset_index(drop=True)
)
df = df.merge(res.head(4), on=['ISIN', 'Name'], how='inner')
res.head(4)
```
...
# Calculating the means vector, and reshaping it to a column vector. 
...
$$
\mathbf{a_{norm}}  = \frac{ {\Sigma}^{-1}({\mu} - \mathbf{1}r) }
