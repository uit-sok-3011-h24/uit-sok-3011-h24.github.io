...
con = pymysql.connect(host='titlon.uit.no', 
                    user="esi000@uit.no", 
                    password="0VDGvZU0h6CEVN2FhjvPz", 
                    database='OSE')  
crsr=con.cursor()
crsr.execute("""
	SELECT 
...
# Assuming df has columns 'Date' and 'OSEBXLinked'
...
bin_width = 0.01
bins = int((df['Returns'].max() - df['Returns'].min()) / bin_width)

# Plot the histogram
plt.figure(figsize=(10, 6))
plt.hist(df['Returns'].dropna(), bins=bins, density=True, edgecolor='k', alpha=0.7)
plt.xlabel('Returns')
plt.ylabel('Probability Density')
plt.title('Histogram of Stock Index Returns (dx = 0.01)')
plt.grid(True)
plt.show()
