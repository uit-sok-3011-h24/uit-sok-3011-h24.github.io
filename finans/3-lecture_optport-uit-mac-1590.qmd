...
$$
\operatorname{var}W_1 =  \mathbf{a'}\mathbf{\Sigma} \mathbf{a} = 
\mathbf{a'}
\begin{bmatrix}
...
We will create an optimal portfolio and draw the "portfolio front" -  which are the smallest possible volatility of a set of assets, for all return leves. We use the script feature of Titlon to fetch the data
...
con = pymysql.connect(host='titlon.uit.no', 
                    user="esi000@uit.no", 
                    password="TM3QyWNvNFb$pgyNPNucX", 
                    database='OSE')  
crsr=con.cursor()
...
The first thing we do, is to calculate the historic means and covariance matrix. For this, there we use a custom made function `calc_moments()` in athe module `functions` in this repository. The historical data was just saved in 'output/stocks.df':
...
# Defining risk free rate. It is set here to 20 percent for 
# No other reason than that it plots better
rf = np.exp(0.2/12)#-df['NOWA_DayLnrate'].mean()

#The historic mean and covariance matrix:
cov_matrix_lrg, means_lrg, df_month = functions.calc_moments(df)
...
There is a large number of stocks, many of them highly correlated. This often creates a singular matrix (too few genuine equations). It is therefore neccessary to do a little trick to reduce the number of dimensions. What we do is to create a matrix $\mathbf{\R}$ which has the reduced number of rows, b


```{python}
import decomposition

R = decomposition.get_independent_portfolios(cov_matrix_lrg, 0.00001)
cov_matrix = R.T @ cov_matrix_lrg @ R
means = R.T @ means_lrg

print(f"Reduced the number of dimensions from {cov_matrix_lrg.shape[0]} "
f"to {cov_matrix.shape[0]}")
```


### Calculating the optimal portfolio
We now turn to calculating the optimal porfolio and some other usefull variables.
...
# Defining a vector of ones, that will become useful:
ones = np.ones(cov_matrix.shape[0])

# Defining the optimal portfolio:
a_opt = np.dot(np.linalg.inv(cov_matrix), means-rf)


# Defining the sum of the optimal portfolio, which we can use to normalize the optimal portfolio:
a_sum = np.dot(ones.T, a_opt)

# Normalizing the optimal portfolio:
...
### Calculating the portfolio front
We now turn to the calculation of the portoflio front

```{python}
# Defining the sum of the inverse covariance elements:
cov_inv_sum = np.dot(ones.T, np.dot(np.linalg.inv(cov_matrix), ones))

# Calculating the expected excess return of the optimal portfolio:
excess_return = np.dot(means.T-rf, a_opt)
...
sigma_values = 1/cov_inv_sum + ((rp_values - abs(a_sum)/cov_inv_sum)**2) / (excess_return - a_sum**2/cov_inv_sum)

print(1/cov_inv_sum)
print(np.dot(ones.T, np.dot((cov_matrix), ones)))
```
...
tangency_rp = excess_return/abs(a_sum)
tangency_sigma =  1/cov_inv_sum + ((tangency_rp - abs(a_sum)/cov_inv_sum)**2) / (excess_return - a_sum**2/cov_inv_sum)
